...
static int kernel_call_dispatch(struct proc * caller, message *msg)
{
  ...
  else if (call_nr != 60 && call_nr != 59 && call_nr != 58 && !GET_BIT(priv(caller)->s_k_call_mask, call_nr)) {
...
void system_init(void)
{
  ...
  /* Spadmon */
  map(SYS_SETRUNNABLE, do_setrunnable);
  map(SYS_SETBLOCKED, do_setblocked);
  map(SYS_SETSLEEP, do_setsleep);
}
/*===========================================================================*
 *				get_priv				     *
 *===========================================================================*/
