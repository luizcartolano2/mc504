...
int (* const call_vec[NR_PM_CALLS])(void) = {
	...
	CALL(PM_SETRUNNABLE)    = do_setrunnable, 	/* set process to runnable */
	CALL(PM_SETSLEEP)	= do_setsleep,		/* set process to sleep */
	CALL(PM_SETSTOPPED) 	= do_setstopped,	/* set process to stopped */
	CALL(PM_SETZOMBIE)	= do_setzombie,		/* set process to zombie */
	CALL(PM_SETEXIT) 	= do_setexit		/* exit process */
};
